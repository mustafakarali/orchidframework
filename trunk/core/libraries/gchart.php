...
class GChart
{
	...
	function GChart($type = 'lc',$encoding = 't')
	{
		$this->setChartType($type);
		$this->setEncoding($encoding);		
	}
	
	function setChartType($type = 'lc')
	{
	...
	function setEncoding($encoding = 't')
	{
	...
	function prepare_styles()
	{
		...
		// colors
		if(is_array($this->chart->chco)) $this->chart->chco = implode(',',$this->chart->chco);
		if(is_array($this->chart->chf)) $this->chart->chf = implode('|',$this->chart->chf);

		// styles
		if($this->chart_type == 'scatter' || $this->chart_type == 'line')
		{
			if($this->chart_type == 'line') if(count($this->chart->chls)) $this->chart->chls = implode('|',$this->chart->chls);
			if(count($this->chart->chg)) $this->chart->chg = implode('|',$this->chart->chg);
			// markers
			if(count($this->chart->chm)) $this->chart->chm = implode('|',$this->chart->chm);
		}
